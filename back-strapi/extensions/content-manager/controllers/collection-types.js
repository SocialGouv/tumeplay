...
const { validateBulkDeleteInput, validatePagination } = require('strapi-plugin-content-manager/controllers/validation');
...
module.exports = {
  async find(ctx) {
    ...
    if (roleNames.includes('Référent')) {
        const referents = await strapi.services.referent.find({'users_in': ctx.state.user.id})

        const where_referent_query = referents.map(r => {
            return {'referent': r.id}
        })

        const where_query = where_referent_query.length > 0 ? {'_or': where_referent_query} : {'delivery': 'hideeverything'}

        permissionQuery._where[0] = Object.assign(permissionQuery._where[0], where_query)
    }
