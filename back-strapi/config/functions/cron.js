...
const getDateFormatted = (date) => {
  const cleanISODate = new Date(date)
    .toISOString()
    .replace(/T/, " ")
    .replace(/\..+/, "");
  ...
  const frenchDate =
    _.get(splittedOnlyDate, "2", "") +
    "/" +
    _.get(splittedOnlyDate, "1", "") +
    "/" +
    _.get(splittedOnlyDate, "0", "");

  return frenchDate + " " + onlyTime;
};

const createColissimoCsv = async (
  dirpath,
  today_7AM,
  orders,
  environnement_id
  ...
  strapi.log.info(
    "CSV COLISSIMO GENERATED WITH PATHS : ",
    csvColissimoPaths.map((r) => r.path).join(" / ")
  );

  return csvColissimoPaths;
};

const createMondialRelayCsv = async (
  dirpath,
  today_7AM,
  orders,
  environnement_id
  ...
  orders.forEach((order) => {
    ...
    if (box.environnement === environnement_id) {
      ...
      csvMondialRelayOrders[box.number].push({
        ...
        date: date,
        address_poi:
          _.get(order, "poi_name") +
          ", " +
          _.get(order, "address") +
          ", " +
          _.get(order, "address_zipcode") +
          " " +
          _.get(order, "address_city"),
        mr_pdf_link:
          strapi.config.get("server.url") +
          "/uploads/orders/mondial-relay/order_mondial_relay_" +
          order.id +
          ".pdf",
      });
    }
  ...
  strapi.log.info(
    "CSV MONDIAL RELAY GENERATED WITH PATHS : ",
    csvMondialRelayPaths.map((r) => r.path).join(" / ")
  );
...
module.exports = {
  ...
  "0 5 * * *": async () => {
    ...
    const environnement_metropole = environnements.find(
      (_) => _.slug === "metropole"
    );
    const environnement_guyane = environnements.find(
      (_) => _.slug === "guyane"
    );
    ...
    const yesterdayFormatted =
      String(yesterday_7AM.getDate()).padStart(2, "0") +
      "/" +
      String(yesterday_7AM.getMonth() + 1).padStart(2, "0") +
      "/" +
      yesterday_7AM.getFullYear();
    const todayFormatted =
      String(today_7AM.getDate()).padStart(2, "0") +
      "/" +
      String(today_7AM.getMonth() + 1).padStart(2, "0") +
      "/" +
      today_7AM.getFullYear();

    const ordersColissimo = await strapi.services.commande.find({
      created_at_gte: yesterday_7AM.getTime(),
      created_at_lt: today_7AM.getTime(),
      delivery: "home",
      sent: false,
      _sort: "created_at:desc",
    });
    const ordersMondialRelay = await strapi.services.commande.find({
      created_at_gte: yesterday_7AM.getTime(),
      created_at_lt: today_7AM.getTime(),
      delivery: "pickup",
      sent: false,
    ...
    const csvColissimoPaths = await createColissimoCsv(
      dirpath,
      today_7AM,
      ordersColissimo,
      environnement_metropole.id
    );
    const csvMondialRelayPaths = await createMondialRelayCsv(
      dirpath,
      today_7AM,
      ordersMondialRelay,
      environnement_metropole.id
    ...
    csvColissimoPaths.forEach((csvColissimoPath) => {
      attachments.push({
        filename:
          "commandes_colissimo_box_" +
          csvColissimoPath.number +
          "_" +
          yesterdayFormatted.split("/").join("") +
          "_" +
          todayFormatted.split("/").join("") +
          ".csv",
        content: fs.createReadStream(csvColissimoPath.path),
      });
    });
    csvMondialRelayPaths.forEach((csvMondialRelayPath) => {
      attachments.push({
        filename:
          "commandes_mondial_relay_box_" +
          csvMondialRelayPath.number +
          "_" +
          yesterdayFormatted.split("/").join("") +
          "_" +
          todayFormatted.split("/").join("") +
          ".csv",
        content: fs.createReadStream(csvMondialRelayPath.path),
      });
    });
  ...
  "0 5 * * *": async () => {
    ...
    await cat_users.forEach((user) => {
      const CAT_TEMPLATE = {
        subject: `Tumeplay: ${
          ordersColissimo.length + ordersMondialRelay.length
        } nouvelle(s) commande(s) prête(s) à être traitées`,
