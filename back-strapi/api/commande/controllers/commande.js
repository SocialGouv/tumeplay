...
const buildMrParams = (order) => {
  ...
  const mrParams = {
    ...
    Dest_Ad1:
      "M. " +
      orderMrParams.fullName
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .replace("-", " ")
        .toUpperCase(),
    Dest_Ad3: orderMrParams.poi_name
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace("-", " ")
      .toUpperCase(),
    Dest_Ad4: orderMrParams.street
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace("-", " ")
      .toUpperCase(),
    Dest_Ville: orderMrParams.city
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace("-", " ")
      .toUpperCase(),
...
const colissimoTmpPdf = async (orders, promises) => {
  ...
  if (orders.length > 0) {
    ...
    const skeletonStyle =
      "display: grid; grid-template-columns: repeat(2, 1fr); margin-left: 0px; margin-right: 0px;";
    const gridItemStyle =
      "color:black; text-align: left; border: solid 1px #dadada; height: 129px; padding-left: 10px; padding-right: 10px;";
    let gridItems = "";
      ...
      gridItems =
        ...
        `
                          <div style="${gridItemStyle}">
                            ...
                            <span>${order.address.toUpperCase()}${
          order.address_more ? ", " + order.address_more.toUpperCase() : ""
        }</span><br><br>
														<span>${order.address_zipcode} ${
          order.address_city && order.address_city.toUpperCase()
        }</span><br>
...
module.exports = {
  async create(ctx) {
    ...
    if (
      ctx.request.body.content[0].__component === "commandes.box-sur-mesure"
    ) {
      const products_box = ctx.request.body.content[0].produits;
      const available = await strapi.services[
        "box-sur-mesure"
      ].checkDynamicBoxAvailability(products_box);

      if (available) {
        products_box.forEach((product_wrapper) => {
          strapi.services["box-sur-mesure"].decrement(
            product_wrapper.produit,
      ...
      const environnement = await strapi.services.environnement.findOne({
        slug: "guyane",
      });
        ...
        if (response_item && response_item.WSI2_CreationEtiquetteResult) {
          ...
          if (mrResult.STAT === "0") {
            ...
            return ctx.methodNotAllowed(
              null,
              "Error " + mrResult.STAT + " while create remote label"
            );
          }
        } else {
          return ctx.internal(
            null,
            "Error while calling mondial relay SOAP service"
          );
        }
      }
    ...
    if (!ctx.request.body.no_email) {
      ...
      const users = await strapi.plugins[
        "users-permissions"
      ].services.user.fetchAll({ referent: referentID });
      ...
      const order_email_txt = await fs.promises.readFile(
      ...
      switch (entity.delivery) {
        case "pickup":
          ...
          custom_text =
            "Nos équipes la préparent et font le maximum pour l’expédier au plus vite. Le n° de suivi de ton colis te sera communiqué par e-mail.";
          break;
        case "home":
        ...
        case "referent":
          ...
          custom_text =
            "Tu pourras récupérer ta boîte dans 5 jours au lieu de rencontre choisi, auprès du référent que tu as choisi.";
          break;
      }
      ...
      if (entity.delivery !== "referent") {
        ...
        const referent_text = await fs.promises.readFile(
          "emails/referent_confirmation.txt",
          "utf8"
        );
        const referent_html = await fs.promises.readFile(
          "emails/referent_confirmation.html",
          "utf8"
        );
        const order_email_ref_txt = await fs.promises.readFile(
          "emails/order_confirmation_ref.txt",
          "utf8"
        );
        const order_email_ref_html = await fs.promises.readFile(
          "emails/order_confirmation_ref.html",
          "utf8"
        );
  ...
  async searchPOI(ctx) {
    ...
    const response = await soapClient.WSI3_PointRelais_RechercheAsync(
      searchPoiParams
    );
    const response_item = response[0];
    ...
    return ctx.badRequest(
      null,
      "Error while trying to fetch mondial relais API"
    );
  },
  async generateMergePDF(ctx) {
    ...
    ids.forEach(async (id) =>
      merger.add(
        path.relative(
          ".",
          `public/uploads/orders/mondial-relay/order_mondial_relay_${id}.pdf`
        )
      )
    );
    const merge_pdf_path =
      "uploads/orders/mondial-relay/merged_" + new Date().getTime() + ".pdf";
  ...
  async createColissimo(ctx) {
    ...
    const merger = new PDFMerger();
    const merge_pdf_path =
      "uploads/orders/colissimo/merged_colissimo_" +
      new Date().getTime() +
      ".pdf";
    await new Promise((resolve) => {
      colissimoTmpPdf(orders, []).then(async () => {
        fs.readdir(relativeDirPath, async (err, files) => {
          files.forEach((file) => {
            merger.add(
              path.relative(".", `public/uploads/orders/colissimo/tmp/${file}`)
            );
          });
          const merge_pdf_save_path = path.relative(
            ".",
            `public/${merge_pdf_path}`
          );
          await merger.save(merge_pdf_save_path);
          resolve();
        });
  ...
  async update(ctx) {
    ...
    if (ctx.request.body.notify_user) {
      const order_referent_notify = await fs.promises.readFile(
        "emails/order_referent_notify.txt",
        "utf8"
      );
      const order_referent_notify_html = await fs.promises.readFile(
        "emails/order_referent_notify.html",
        "utf8"
      );
