...
module.exports = {
    ...
    let user = await strapi.services['utilisateurs-mobile'].findOne({ user_id: id });
		const history = await strapi.services['historique'].find({user: user.id}, ['module.niveau'])
		...
		const history_modules_by_levels = _.groupBy(success_modules, 'niveau.value')

		let user_level = 1
		for (const [level, user_modules] of Object.entries(history_modules_by_levels)) {
