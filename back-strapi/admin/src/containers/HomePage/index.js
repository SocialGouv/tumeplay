...
const HomePage = ({ global: { plugins }, history: { push } }) => {
  ...
  const [quizTimeByIterationsMetropole, setQuizTimeByIterationsMetropole] =
    useState([]);
  const [
    quizAverageScoreByThemesMetropole,
    setQuizAverageScoreByThemesMetropole,
  ] = useState([]);
  const [quizAverageScoreByThemesGuyane, setQuizAverageScoreByThemesGuyane] =
    useState([]);
  const [quizTimeUsedThemesMetropole, setQuizTimeUsedThemesMetropole] =
    useState([]);
  ...
  const [quizAverageScoreByAgeMetropole, setQuizAverageScoreByAgeMetropole] =
    useState([]);
  const [quizAverageScoreByAgeGuyane, setQuizAverageScoreByAgeGuyane] =
    useState([]);

  const [
    quizAverageScoreByIterationMetropole,
    setQuizAverageScoreByIterationMetropole,
  ] = useState([]);
  const [
    quizAverageScoreByIterationGuyane,
    setQuizAverageScoreByIterationGuyane,
  ] = useState([]);
  ...
  const [switchZoneQuizTimeByIteration, setSwitchZoneQuizTimeByIteration] =
    useState("metropole");
  const [
    switchZoneQuizAverageScoreByThemes,
    setSwitchZoneQuizAverageScoreByThemes,
  ] = useState("metropole");
  const [switchZoneQuizAverageScoreByAge, setSwitchZoneQuizAverageScoreByAge] =
    useState("metropole");
  const [
    switchZoneQuizAverageScoreByIteration,
    setSwitchZoneQuizAverageScoreByIteration,
  ] = useState("metropole");
  const [switchZoneTop10Questions, setSwitchZoneTop10Questions] =
    useState("metropole");
  const [switchZoneFlop10Questions, setSwitchZoneFlop10Questions] =
    useState("metropole");
  ...
  const default_params = {
    created_at_gte:
      dateRanges[0].startDate && dateRanges[0].startDate.getTime(),
  ...
  const fetchResponses = async () => {
    ...
    const data = await request("/reponses", {
      method: "GET",
      params: default_params.created_at_gte
        ? Object.assign({ _limit: 10000 }, default_params)
        : { _limit: 10000 },
    });
      ...
      if (theme.environnement.slug === "metropole") {
        tmpResponsesMetropole.push({
          ...
          percentageRightAnswer:
            (countRightAnswers / (countRightAnswers + countBadAnswers)) * 100,
        });
    ...
    setTop10QuestionsMetropole(
      orderedQuestionsMetropole
        .orderBy(
          ["percentageRightAnswers", "occurences", "question"],
          ["desc", "desc", "asc"]
        )
        .splice(0, 9)
        .value()
    ...
    setTop10QuestionsGuyane(
      orderedQuestionsGuyane
        .orderBy(["percentageRightAnswers", "question"], ["desc", "asc"])
        .splice(0, 9)
        .value()
    );

    setFlop10QuestionsGuyane(
      orderedQuestionsGuyane
  ...
  const fetchQuizTimes = async () => {
    ...
    const countLowerThan30s = await request("/quiz-times/count", {
      method: "GET",
      params: default_params.created_at_gte
        ? Object.assign({ nb_seconds_lte: 30 }, default_params)
        : { nb_seconds_lte: 30 },
    });
    ...
    const dataGroupedByThemes = _(data)
      .groupBy((x) => x.thematique.id)
      .map((value, key) => ({
        thematique: key,
        data: value,
        environnement: value[0].thematique.environnement,
        title: value[0].thematique.title,
      }))
      .value();
    ...
    iterations.forEach((i) => {
      ...
      dataGroupedByThemes.forEach((dgbt) => {
        const iterationData = _.filter(
          dgbt.data,
          (d) => d.quizz_iteration === i
        );
        if (iterationData.length) {
          if (dgbt.environnement === 1) {
            pointTimeMetropole[dgbt.title] = _.meanBy(iterationData, (d) =>
              parseInt(d.nb_seconds)
            );
            usedThemesMetropole.push(dgbt.title);
          } else if (dgbt.environnement === 2) {
            pointTimeGuyane[dgbt.title] = _.meanBy(iterationData, (d) =>
              parseInt(d.nb_seconds)
            );
    ...
    iterations.forEach((i) => {
      ...
      dataGroupedByIterationsMetropole.forEach((dgbi) => {
        ...
        if (iterationData.length) {
          pointScoreMetropole["score"] = _.meanBy(iterationData, (d) =>
            parseInt(d.score)
          );
        }
      });
      ...
      dataGroupedByIterationsGuyane.forEach((dgbi) => {
        ...
        if (iterationData.length) {
          pointScoreGuyane["score"] = _.meanBy(iterationData, (d) =>
            parseInt(d.score)
          );
        }
      });
    ...
    setQuizAverageScoreByIterationMetropole(
      tmpAverageScoreByIterationMetropole
    );
  ...
  useEffect(() => {
    if (showRangeModal)
      setTmpDateRanges(
        dateRanges[0].startDate ? dateRanges : [default_range_date]
      );
  }, [showRangeModal]);
      ...
      <>
        <button
          className={`switcher-button ${
            variable === "metropole" ? "active" : ""
          }`}
          onClick={() => setter("metropole")}
        >
          Métropole
        </button>
        <button
          className={`switcher-button ${variable === "guyane" ? "active" : ""}`}
          onClick={() => setter("guyane")}
        >
          Guyane
        </button>
      </>
  ...
  const repeatRequest = async (data, route, exportParams, start = 0) => {
    let response = await request("/" + route, {
      method: "GET",
      params: Object.assign(exportParams, {
        _start: start,
      }),
    });
  ...
  const exportData = async (type) => {
    ...
    switch (type) {
      case "contents":
        route = "contents";
        csvData.push('"Titre";"Contenu";"Thématique"');
        break;
      case "questions":
        route = "questions";
        csvData.push(
      ...
      case "contacts":
        route = "contacts";
        csvData.push('"Nom";"Email";"Code postal";"Box";"Zone";"Type"');
        break;
      case "commandes":
        route = "commandes";
          ...
          csvData.push(
            '"' +
              (item.text_question
                ...
                : "") +
              '";"' +
              (item.responses && item.responses.response_A
                ? item.responses.response_A.replaceAll('"', "'")
                : "") +
              '";"' +
              (item.responses && item.responses.response_B
                ? item.responses.response_B.replaceAll('"', "'")
                : "") +
              '";"' +
              (item.responses && item.responses.response_C
                ? item.responses.response_C.replaceAll('"', "'")
                : "") +
              '";"' +
              (item.responses && item.responses.right_answer
              ...
              '";"' +
              (item.sound_queston
                ? item.sound_queston.hash.replaceAll('"', "'")
                : "") +
              '";"' +
              (item.sound_answer
                ? item.sound_answer.hash.replaceAll('"', "'")
                : "") +
              '";"' +
              (item.theme && item.theme.title
                ? item.theme.title.replaceAll('"', "'")
          ...
          if (userRole.includes("pilote-guyane")) {
            csvData.push(
              '"' +
                ...
                '";"' +
                (item.user_data && item.user_data.sex
                  ? item.user_data.sex === "man"
                    ? "Garçon"
                    : "Fille"
                  : "") +
                '";"' +
                (item.user_data && item.user_data.age
                  ? item.user_data.age
                  : "") +
                '";"' +
                (item.user_data && item.user_data.district
                  ? item.user_data.district
                  : "") +
                '";"' +
                (item.user_data && item.user_data.zipcode
                  ? item.user_data.zipcode
                  : "") +
                '";"' +
                (item.user_data && item.user_data.city
                  ? item.user_data.city
                  : "") +
                '";"' +
                (item.user_data && item.user_data.schooling
                  ? item.user_data.schooling
                  : "") +
                '";"' +
                (item.user_data && item.user_data.comment
                  ? item.user_data.comment
                  : "") +
                '";"' +
                (item.user_data && item.user_data.old_box
                  ? _.get(
                      boxes.find(
                        (box) => box.number === parseInt(item.user_data.old_box)
                      ),
                      "title",
                      item.user_data.old_box
                    )
                  : "") +
                '";"' +
                (item.user_data && item.user_data.first_box
                  ? item.user_data.first_box
                  : "") +
                '";"' +
    ...
    element.setAttribute(
      "href",
      "data:text/plain;charset=utf-8," + encodeURIComponent(csvData.join("\n"))
    );
    element.setAttribute(
      "download",
      type + "_" + new Date().getTime() + ".csv"
    );
      ...
      <Container className="container-fluid">
        <div className="row">
            ...
            {dateRanges[0].startDate && dateRanges[0].endDate && (
              <>
                <div className="selected-date-preview">
                  Du {formatDate(dateRanges[0].startDate)} au{" "}
                  {formatDate(dateRanges[0].endDate)}
                </div>
                <button
                  className="button button-default"
                  onClick={() => setDateRanges([null_range_date])}
                >
                  Annuler la période
                </button>
              </>
          ...
          <div className="col-3 text-center">
            <Block>
              <h2 className="mt-4">{nbCompletedQuiz}</h2>
              <p>Quiz complétés</p>
            </Block>
          </div>
          <div className="col-3 text-center">
            <Block>
              <h2 className="mt-4">{nbResponses}</h2>
              <p>Réponses aux questions</p>
            </Block>
          </div>
          <div className="col-3 text-center">
            <Block>
              <h2 className="mt-4">{nbOrders7days}</h2>
          ...
          <div className="col-7">
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <div className="zone-switcher">
                {ZoneSwitcher(
                  switchZoneQuizTimeByIteration,
                  setSwitchZoneQuizTimeByIteration
                )}
              </div>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  ...
                  height={300}
                  data={
                    switchZoneQuizTimeByIteration === "metropole"
                      ? quizTimeByIterationsMetropole
                      : quizTimeByIterationsGuyane
                  }
                  margin={{
                    top: 20,
                    right: 30,
                ...
                >
                  ...
                  <Legend />
                  {(switchZoneQuizTimeByIteration === "metropole"
                    ? quizTimeUsedThemesMetropole
                    : quizTimeUsedThemesGuyane
                  ).map((theme_name) => {
                    return (
                      <Line
                        key={theme_name}
            ...
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  ...
                  height={300}
                  data={
                    switchZoneQuizAverageScoreByIteration === "metropole"
                      ? quizAverageScoreByIterationMetropole
                      : quizAverageScoreByIterationGuyane
                  }
                  margin={{
          ...
          <div className="col-4">
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <div className="zone-switcher">
                {ZoneSwitcher(
                  switchZoneQuizAverageScoreByAge,
                  setSwitchZoneQuizAverageScoreByAge
                )}
              </div>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  ...
                  data={
                    switchZoneQuizAverageScoreByAge === "metropole"
                      ? quizAverageScoreByAgeMetropole
                      : quizAverageScoreByAgeGuyane
                  }
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <Tooltip />
            ...
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  ...
                  data={
                    switchZoneQuizAverageScoreByThemes === "metropole"
                      ? quizAverageScoreByThemesMetropole
                      : quizAverageScoreByThemesGuyane
                  }
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <Tooltip />
          ...
          <div className="col-6">
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <div className="zone-switcher">
                {ZoneSwitcher(
                  switchZoneTop10Questions,
                  setSwitchZoneTop10Questions
                )}
              </div>
              <div className="simple-table">
                ...
                {(switchZoneTop10Questions === "metropole"
                  ? top10QuestionsMetropole
                  : top10QuestionsGuyane
                ).map((q) => {
                  return (
          ...
          <div className="col-6">
            <Block style={{ height: "500px", padding: "4rem 3rem 6rem 3rem" }}>
              ...
              <div className="zone-switcher">
                {ZoneSwitcher(
                  switchZoneFlop10Questions,
                  setSwitchZoneFlop10Questions
                )}
              </div>
              <div className="simple-table">
                <div className="simple-table-row">
                ...
                {(switchZoneFlop10Questions === "metropole"
                  ? flop10QuestionsMetropole
                  : flop10QuestionsGuyane
                ).map((q) => {
                  return (
                    <div key={q.question} className="simple-table-row">
                      <div className="simple-table-col question-text">
